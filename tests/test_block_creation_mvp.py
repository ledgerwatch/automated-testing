...
from errors import wrong_payload_errors_legacy, wrong_payload_errors_eip1559, wrong_fields_errors_common, \
    wrong_fields_errors_legacy, wrong_fields_errors_eip1559
...
class TestMVPTestCase(MVPTestCase):
    ...
    @pytest.mark.xfail(reason="asd", run=False)
    @pytest.mark.parametrize(
        "number_of_transactions", [1, 10])
    @pytest.mark.parametrize(
    ...
    def test_valid_transactions(self, number_of_transactions, transaction_template):
        ...
        for tx_count in range(number_of_transactions - 1):
            transaction_dict = transaction_template(self.web3, self.receiver_address, self.sender_private_key)
        ...
        successful_transactions.append(
            self.send_transaction(
                transaction_template(self.web3, self.receiver_address, self.sender_private_key)
            )
        )
        ...
        blocks_to_fit_transactions = math.ceil(number_of_transactions / (initial_block["gasLimit"] / 21000))
        blocks_diff = latest_block_number["number"] - initial_block["number"]

        assert blocks_diff >= blocks_to_fit_transactions, f"It took {blocks_diff} to fit {number_of_transactions} " \
    ...
    @pytest.mark.parametrize(
        "gas, status", [
            (200000, 0),  # failed contract
            (5000000, 1)  # successful contract
        ]
    )
    def test_transaction_contract_deployment(self, gas, status):
        transaction_hash = self.send_transaction(
            ContractHelper.deploy_contract(self.web3, self.sender_private_key, contract_path='/ERC20_token.sol',
                                           gas=gas)
        )
        self.wait_for_transaction_to_be_removed_from_pool(transaction_hash)
        assert self.web3.eth.get_transaction_receipt(transaction_hash)['status'] == status, \
    ...
    def test_transaction_send_erc20tokens(self):
        ...
        receiver_balance_before = contract.functions.balanceOf(self.receiver_address).call()
        ...
        signed_txn = self.web3.eth.account.signTransaction(raw_txn, self.sender_private_key)
        token_transaction_hash = self.web3.eth.sendRawTransaction(signed_txn.rawTransaction).hex()

        self.wait_for_transaction_to_be_removed_from_pool(token_transaction_hash)
        assert self.web3.eth.get_transaction_receipt(token_transaction_hash)['status'] == 1, \
        ...
        receiver_balance_after = contract.functions.balanceOf(self.receiver_address).call()
    ...
    @pytest.mark.parametrize("transaction_template,data_set",
                             generate_tests(
    ...
    def test_transaction_with_wrong_field_values(self, transaction_template, data_set):
        template = form_template(
            transaction_template=transaction_template(self.web3, self.receiver_address, self.sender_private_key),
            fields_to_update=data_set['fields']
        )
        try:
            self.send_transaction(template)
            raise AssertionError(f"Transaction is sent with fields: {data_set['fields']}")
        except ValueError as e:
            assert all(
                (
                    e.args[0]['code'] == -32000,
                    e.args[0]['message'] == data_set['error']
                )
            ), f"Got error {str(e)}"
